...
import com.example.gestion_appliances_v2.bean.*;
...
@Service
public class PovAdminServiceImpl extends AbstractServiceImpl<Pov> implements PovAdminService {
    ...
    public Pov save(Pov entity) {
        ...
        if(foundedPov == null){
            ...
            Appliance appliance = applianceAdminService.findByIdOrReference(entity.getAppliance());
    ...
    @Override
    public Pov archiver(Pov pov) {
        if (pov.getArchive() == null) {
            pov.setArchive(false);
        }
        pov.setArchive(true);
        pov.setDateArchivage(new Date());
        povDao.save(pov);
        return pov;
    }

    @Override
    public Pov desarchiver(Pov pov) {
        if (pov.getArchive() == null) {
            pov.setArchive(false);
        }
        pov.setArchive(false);
        pov.setDateArchivage(null);
        povDao.save(pov);
        return pov;
    }
    ...
    private void findAssociatedLists(Pov pov) {
        if (pov != null && pov.getId() != null) {
            Appliance appliance = applianceAdminService.findByIdOrReference(pov.getAppliance());
            pov.setAppliance(appliance);
            Client client = clientAdminService.findByIdOrLibelle(pov.getClient());
