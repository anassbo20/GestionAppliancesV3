...
@Service
public class TypeApplianceAdminServiceImpl extends AbstractServiceImpl<TypeAppliance> implements TypeApplianceAdminService {
    @Autowired
    ...
    @Override
    public TypeAppliance archiver(TypeAppliance typeAppliance) {
        if (typeAppliance.getArchive() == null) {
            typeAppliance.setArchive(false);
        }
        typeAppliance.setArchive(true);
        typeAppliance.setDateArchivage(new Date());
        typeApplianceDao.save(typeAppliance);
        return typeAppliance;
    }

    @Override
    public TypeAppliance desarchiver(TypeAppliance typeAppliance) {
        if (typeAppliance.getArchive() == null) {
            typeAppliance.setArchive(false);
        }
        typeAppliance.setArchive(false);
        typeAppliance.setDateArchivage(null);
        typeApplianceDao.save(typeAppliance);
        return typeAppliance;
    }
    ...
    private void deleteAssociatedLists(Long id) {
        if (id != null) {
            applianceAdminService.deleteByTypeApplianceId(id);
        }
    }
}
