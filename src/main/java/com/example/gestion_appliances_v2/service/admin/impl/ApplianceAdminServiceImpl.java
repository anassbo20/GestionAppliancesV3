...
@Service
public class ApplianceAdminServiceImpl extends AbstractServiceImpl<Appliance> implements ApplianceAdminService {
    @Autowired
    ...
    @Override
    public Appliance archiver(Appliance appliance) {
        if (appliance.getArchive() == null) {
            appliance.setArchive(false);
        }
        appliance.setArchive(true);
        appliance.setDateArchivage(new Date());
        applianceDao.save(appliance);
        return appliance;
    }

    @Override
    public Appliance desarchiver(Appliance appliance) {
        if (appliance.getArchive() == null) {
            appliance.setArchive(false);
        }
        appliance.setArchive(false);
        appliance.setDateArchivage(null);
        applianceDao.save(appliance);
        return appliance;
    }
    ...
    private void findAssociatedLists(Appliance appliance) {
        if (appliance != null && appliance.getId() != null) {
            TypeAppliance typeAppliance = typeApplianceAdminService.findByLibelle(appliance.getTypeAppliance().getLibelle());
            appliance.setTypeAppliance(typeAppliance);
            List<ApplianceTag> applianceTags = applianceTagAdminService.findByApplianceId(appliance.getId());
            appliance.setApplianceTags(applianceTags);
        }
    }
    private void deleteAssociatedLists(Long id) {
        if (id != null) {
            applianceTagAdminService.deleteByApplianceId(id);
        }
    }
