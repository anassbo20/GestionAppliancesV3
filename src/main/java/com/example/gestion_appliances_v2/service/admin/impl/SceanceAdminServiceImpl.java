...
@Service
public class SceanceAdminServiceImpl extends AbstractServiceImpl<Sceance> implements SceanceAdminService {
    ...
    @Override
    public Sceance archiver(Sceance sceance) {
        if (sceance.getArchive() == null) {
            sceance.setArchive(false);
        }
        sceance.setArchive(true);
        sceance.setDateArchivage(new Date());
        sceanceDao.save(sceance);
        return sceance;
    }

    @Override
    public Sceance desarchiver(Sceance sceance) {
        if (sceance.getArchive() == null) {
            sceance.setArchive(false);
        }
        sceance.setArchive(false);
        sceance.setDateArchivage(null);
        sceanceDao.save(sceance);
        return sceance;
    }
    ...
    private void findAssociatedLists(Sceance sceance) {
        if (sceance != null && sceance.getId() != null){
            Pov pov=povAdminService.findByIdOrLibelle(sceance.getPov());
            sceance.setPov(pov);
        }
    }
    ...
    public Sceance save(Sceance entity) {
        ...
        if(foundedSceance == null){
            ...
            Pov pov=povAdminService.findByIdOrLibelle(entity.getPov());
            entity.setPov(pov);
