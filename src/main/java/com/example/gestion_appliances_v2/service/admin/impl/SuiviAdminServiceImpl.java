...
@Service
public class SuiviAdminServiceImpl extends AbstractServiceImpl<Suivi> implements SuiviAdminService {
    ...
    public Suivi save(Suivi entity) {
        ...
        if(foundedSuivi == null){
            ...
            Pov pov = povAdminService.findByLibelle(entity.getPov().getLibelle());
            entity.setPov(pov);
    ...
    private void findAssociatedLists(Suivi suivi) {
        if (suivi != null && suivi.getId() != null) {
            Pov pov = povAdminService.findByLibelle(suivi.getPov().getLibelle());
            suivi.setPov(pov);
