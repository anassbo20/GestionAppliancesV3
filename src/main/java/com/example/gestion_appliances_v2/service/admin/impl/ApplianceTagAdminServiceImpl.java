...
@Service
public class ApplianceTagAdminServiceImpl extends AbstractServiceImpl<ApplianceTag> implements ApplianceTagAdminService {
    ...
    private void findAssociatedLists(ApplianceTag applianceTag) {
        if (applianceTag != null && applianceTag.getId() != null) {
            Appliance appliance = applianceAdminService.findByIdOrReference(applianceTag.getAppliance());
            applianceTag.setAppliance(appliance);
            Tag tag =  tagAdminService.findByIdOrReference(applianceTag.getTag());
            applianceTag.setTag(tag);
        }
    }

    private void findAppliance(ApplianceTag applianceTag){
        Appliance loadedAppliance =applianceAdminService.findByIdOrReference(applianceTag.getAppliance());
    ...
    private void findTag(ApplianceTag applianceTag){
     Tag loadedTag =tagAdminService.findByIdOrReference(applianceTag.getTag());
